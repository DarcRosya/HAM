...
function startWatchdog() {
  ...
  watchdogTimer = setTimeout(() => {
    console.warn('[NETWORK] Сервер не ответил вовремя. Соединение мертво.');
...
export const BattleNetwork = {
  init(injectedCallbacks) {
    ...
    if (!socket) {
      console.error('[NETWORK] Критическая ошибка: socket is null при подключении');
      return;
    }

    // Защита от дублей: снимаем старые лисенеры ТОЛЬКО с этого сокета
  ...
  cleanup() {
    ...
    // Снимаем слушатели перед отключением
    socket.off('error', handleError);
    socket.off('match_found', handleMatchFound);
    socket.off('force-reconnect', handleForceReconnect);
    socket.off('game_state', handleGameState);
